...
static char * spx_skp_serial_which_file(){/*{{{*/
    char *file = (char*)calloc(1, sizeof(char) * SpxSkpSerialFileNameSize);// yyyy/mm/dd\n
    char path[SpxSkpSerialFileNameSize];
    char path1[SpxSkpSerialFileNameSize];
...
int64_t spx_block_skp_node_serial(struct spx_block_skp *block_skp, void *key, void * value, int64_t index){/*{{{*/
    ...
    if (0 == flag){
        ...
        if (SpxSkpSerialBlockSize - index_len < item_len){
            int mid_off = spx_block_skp_node_less_off_detect(block_mst->mapped, index_len, mid_size);
            if (-1 == mid_off){
                goto r1;
            }
        ...
        if (0 == split_flag){
            ...
            if(1 == split_flag){
                //set old right key
                int mid_off = index_len - tmp_index_len;
                int old_right_off = spx_block_skp_node_less_off_detect(block_mst->mapped, mid_off, mid_off);//mid_off is the new block's length
                int old_right_key_len = spx_msg_b2i(block_mst->mapped + old_right_off);
                void *old_right_key = byte2key(block_mst->mapped + old_right_off + 4, old_right_key_len); 
                g_spx_block_skp_node_serial_ctx.old_right_key = old_right_key;
...
static int spx_block_skp_serial_value_node_query(char *path, ubyte_t *header, int64_t index, SpxSkpB2ODelegate byte2value, struct spx_skp_query_result *result){/*{{{*/
    ...
    if (true == spx_skp_serial_is_index_available(next_idx)){
        return spx_block_skp_serial_value_node_query(path, header, next_idx, byte2value, result); 
    }
...
struct spx_skp_query_result *spx_block_skp_serial_node_query(struct spx_block_skp *block_skp, void *key, int64_t index){/*{{{*/
    ...
    int off = 0;
    int flag = spx_block_skp_node_key_detect(block_mst->mapped, index_len, &off, block_skp->cmp_key, key, byte2key, block_skp->free_key);
    struct spx_skp_query_result *result = spx_skp_query_result_new();
    if (flag ==0){
        ubyte_t * key_ptr = block_mst->mapped + off;
        int key_len = spx_msg_b2i(key_ptr);
        int64_t value_index = spx_msg_b2l(key_ptr + 4 + key_len);
        ...
        spx_skp_query_result_insert(result, value);
        if (value_index != -1){
            spx_block_skp_serial_value_node_query(path, header, value_index, byte2value, result);
        }
    }
